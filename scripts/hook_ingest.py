...
Reads JSON payload from stdin, walks the transcript JSONL backwards to find
the last user prompt + last assistant response, and stores them as a single
episode in the timegraph. Future UserPromptSubmit hooks can then recall this
turn semantically.

Fails open: exits 0 silently on any error.
"""
...
def main() -> None:
    ...
    try:
        asyncio.run(_ingest(combined, session_id, args.group_id))
    except Exception:
        return
